...
flowchart TD
    ...
    C --> D["CV 解析 boundary / district polygons / weight hints"]
    D --> E["程序做基础一致性检查"]
    E --> F["C2 多边形功能区图 + 权重表"]
    F --> G["C3 ownership / block"]
    G --> H["C4 道路求解"]
    H --> I["C5 parcel / area"]
    I --> S["单功能区切到 step=1 精细底图"]
    S --> J["C6 footprint budget"]
    J --> K["C7 工头计划"]
...
| C4 | 道路求解 | 在城市边界和功能区基础上处理主路、支路、桥、广场连接 |
| C5 | parcel / area | 在街坊内切出可施工 parcel，保留不规则 polygon |
| C6 | footprint budget | 单功能区进入 `step = 1` 后，结合结构池 footprint 分布生成局部空间预算 |
| C7 | 工头计划 | 基于功能区、结构画像库和局部底图生成工头计划 |
...
9. 用同一座城市对比旧 C1 与新 C1.5 的城市轮廓自然度、功能区合理性和后续 C6-C9 成功率。
